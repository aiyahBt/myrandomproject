...
from user.utilityFunction import redirect_to_home_something_went_wrong
...
def search_isbn_matching(request, isbn_13_int):
    ...
    shelf_query = models.User_Book.objects.filter(userID=request.user.id, available=True)
    ...
    if not (user_book_query.exists()):  # No users own this book.
        ...
        user_matched_from_wish_list = models.Wish_List.objects.filter(userID__in=users_own_this_book).filter(
            isbn_13__in=shelf_isbn_list).first()  # Limit =
...
def search_isbn(request):
    ...
    if (search_option == '1'):  # Search within database. (search in Book table.)
        ...
        if (book_query.exists()):
            ...
            book = ''
            # Assign values to book. And create book obj.
            try:
